import numpy as np
import cv2
from CabezaFemur import cabezaFemurProximal
...
def detectar(tomografia_segmentada,tomografia_original):
    ...
    encontreProximalIzquierdo=False
    ...
    for i in range(tomografia_segmentada.shape[2] - 1, -1, -1):
        ...
        if(encontreProximalDerecho and encontreProximalIzquierdo):
            break
       ...
       # if(contador_sali_de_cabeza_femur_derecho==10):
        ...
        if existe_contorno_circular_izquierdo and encontreProximalIzquierdo is False:

            flag_cabeza_femur_izquierda,cabeza_izquierda = cabezaFemurProximal.detectar(femur_izquierdo_segmentado,femur_izquierdo_segmentado_gris,contorno_mas_grande_izquierdo,tomografia_original[:, :, i])
            ...
            if (flag_cabeza_femur_izquierda):
                ...
                if( r_izq >= radio_maximo_izquierdo): #Me quedo con la primera aparicion del mas grande o con la ultima?
                    ...
                    contador_sali_de_cabeza_femur_izquierdo=0
                    encontreProximalIzquierdo=True
...
#                print(f"No hallo cabeza de femur en el corte: {i+1}")
        ...
        if existe_contorno_circular_derecho and encontreProximalDerecho is False:

            flag_cabeza_femur,cabeza_derecha = cabezaFemurProximal.detectar(femur_derecho_segmentado,femur_derecho_segmentado_gris,contorno_mas_grande_derecho,tomografia_original[:, :, i])
