...
def graficar(coronal_slice, x, y, x_opuesto, y_opuesto, x_final, y_final, lado):
    ...
    # Punto verde en (x, y)
    cv2.circle(coronal_slice, (x, y), 1, (0, 255, 0), 2)
    # Punto verde en (x_opuesto, y_opuesto)
    cv2.circle(coronal_slice, (x_opuesto, y_opuesto), 1, (0, 255, 0), 2)
...
def detectar(id, base_path, cabezas_femur_axiales, tomografia_original, tomografia_segmentada):
    ...
    x_izq, y_izq, z_izq = esCorteEcuatorialCoronal.obtenerCentroide(
        tomografia_original, coordenadas_izq, numero_corte_izq)
    coronal_slice_izq_segmentado = tomografia_segmentada_invertida[:, z_izq, :, 5]
    coronal_slice_izq_segmentado = preprocesarCoronalSegmentado.procesarCorte(
    ...
    x_final_izq, y_final_izq, angulo_CBL_izq,_ = aasa.detectar(
        coronal_slice_izq_segmentado, x_izq, y_izq, x_der, y_der, 1, "izquierdo")
    x_final_der, y_final_der, angulo_CBL_der,_ = aasa.detectar(
        coronal_slice_der_segmentado, x_der, y_der, x_izq, y_izq, 1, "derecho")

    corte_CBL_izquierdo = graficar(
        coronal_slice_izq, x_izq, y_izq, x_der, y_der, x_final_izq, y_final_izq, "izquierdo")
    corte_CBL_derecho = graficar(
        coronal_slice_der, x_der, y_der, x_izq, y_izq, x_final_der, y_final_der, "derecho")

    # Cortar cada imagen por la mitad
    mitad_derecha_izq = corte_CBL_izquierdo[:,
                                            corte_CBL_izquierdo.shape[1] // 2:]
    mitad_izquierda_der = corte_CBL_derecho[:,
                                            :corte_CBL_derecho.shape[1] // 2]
