...
def detectar(imagen_HU, x1_centroide, y1_centroide, x2_centroide_opuesto, y2_centroide_opuesto, radio, lado='derecho'):
    if lado == 'derecho':
        angulo_inicial = 90  # Ángulo inicial en grados para el lado derecho
        angulo_incremento = -1  # Decremento del ángulo para el lado derecho
    else:
        angulo_inicial = -90  # Ángulo inicial en grados para el lado izquierdo
    ...
    while abs(angulo_inicial) >= 0:
        ...
        for t in np.linspace((radio+3) / longitud_linea, 1, max(abs(x_final - x1_centroide), abs(y_final - y1_centroide))):
